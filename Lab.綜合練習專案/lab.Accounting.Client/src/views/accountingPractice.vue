...
const deleteChange=async(id)=>{
  if(!id)return;
   const res=await fetch(`https://localhost:7124/api/Account/DeleteLedger/${id}`,{method:'Delete'});
...
if(!item)return;
const res=await fetch(`https://localhost:7124/api/Account/UpdateLedger?categoryname=${item.categoryName || ''}`,{method:'Put',headers: {
      ...
      <div style="margin-top: 50px">
        <div class="card max-w-6xl mx-auto px-10">
          <DataTable :value="products" scrollable scrollHeight="430px" size="large">
            ...
            <Column field="itemCost" header="花費"><template #body="slotProps">
          ...
          {{ slotProps.data.itemCost ? slotProps.data.itemCost : 0 }}
            ...
            <Column field="
itemCreateDate
" header="建立日期"><template #body="slotProps">

          {{ slotProps.data.itemCreateDate ? slotProps.data.itemCreateDate.split('T')[0] : '' }}

      </template></Column>
            <Column header="狀態"><template #body="slotProps">
