...
const categoryData=async()=>{
  try
  {
    let url=`https://localhost:7124/api/Account/GetAllLedger`;
...
const updateData=async(id)=>{
  ...
  if (!id) return;
 let url=`https://localhost:7124/api/Account/GetLedger?ledgerId=${id}`;
...
const addaccounledger = async () => {
    ...
    try
    {
      ...
      const categoryname=typeof selectedCategory.value==='object'?selectedCategory.value.name:selectedCategory.value;


const res=await fetch(`https://localhost:7124/api/Account/CreateLedger?categoryname=${categoryname}`,{method:'Post',headers: {
      'Content-Type': 'application/json'
    },body:JSON.stringify({
        ItemName: itemName.value,
        UserId: userId.value,
        // 轉日期格式
        ItemCreateDate:itemDate.value ? new Date(itemDate.value).toLocaleDateString('en-CA') : null,
